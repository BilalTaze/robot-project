...
import threading
import time
from log import init_log, log_entry
...
class RobotVoiceApp:
    ...
    def __init__(self, model_name="base"):
        ...
        self.btn_confirm_canvas.bind("<Button-1>", lambda e: self.command_confirmed())
    ...
    def process_voice(self):
        ...
        try:
            ...
            # Start the chrono just before Whisper transcription
            start = time.time()
            ...
            # Stop the chrono after transcription
            latency_ms = round((time.time() - start) * 1000, 2)

            # Log the entry: robot_action is "pending_confirmation" until user confirms
            log_entry("whisper_audio", self.command, "pending_confirmation", latency_ms)
    ...
    def update_ui(self, result):
        ...
        if 'Error' in str(result) or result == "":
            ...
            self.confirm_timer = self.root.after(4000, self.command_confirmed)
    ...
    def command_confirmed(self):
        ...
        if hasattr(self, 'confirm_timer'):
            self.root.after_cancel(self.confirm_timer)
        print(f'Command "{self.command}" confirmed, sending to robot...')
        # Fonction pour envoyer la commande au robot (à implémenter)
